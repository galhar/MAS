...
def interpolate_motion(motion, frames_to_interpolate):
    ...
    motion_vel = torch.cat(
        [motion_vel, torch.zeros((1, motion.shape[1], motion.shape[2]))], dim=0
    )
    motion_lengthened = torch.zeros(
        (motion.shape[0] * frames_to_interpolate, motion.shape[1], motion.shape[2])
    )
    for i in range(frames_to_interpolate):
        motion_lengthened[i::frames_to_interpolate] = (
            motion + (i / frames_to_interpolate) * motion_vel
        )
...
def project_to_random_angle(motion, dataset, mode):
    ...
    projected_motion = (
        perspective_projection(motion, [hor_angle], [ver_angle], distance)[0]
        * distance
        / get_visualization_scale(dataset)
    )
...
class Evaluator:
    ...
    def get_data_samples(self, split, **kwargs):
        return list(
            get_dataset_loader(
                self.args.dataset,
                split=split,
                data_size=self.args.eval_num_samples,
    ...
    def apply_on_data(self, func, split, **kwargs):
        return [
            func(input_motion, model_kwargs)
            for input_motion, model_kwargs in tqdm(
                self.get_data_samples(split, **kwargs)
            )
        ]
    ...
    def encode_samples(self, iter):
        return [
            self.encode(motion, model_kwargs) for motion, model_kwargs in tqdm(iter)
        ]
    ...
    def get_mas_samples(self, mas_type_name, model_args, split="test"):
        ...
        def generate_motion(input_motion, model_kwargs):
            ...
            samples_2d = torch.stack(
                [
                    project_to_random_angle(
                        sample_3d, self.args.dataset, self.args.angle_mode
                    )
                    for sample_3d in samples_3d
                ]
            ).permute(0, 2, 3, 1)
    ...
    def get_3d_samples(self, samples_path, scale=1, flip=False, fps_ratio=1):
        ...
        random.shuffle(
            file_names
        )  # Shuffling instead of random sampling can increase diversity and recall. Should not hurt FID and precision by much.
        ...
        motions_batched = [
            zip(
                motions[i : i + self.args.batch_size],
                lengths[i : i + self.args.batch_size],
                masks[i : i + self.args.batch_size],
            )
            for i in range(0, len(motions), self.args.batch_size)
        ]
        motions_batched = [collate(batch) for batch in motions_batched]
        motions_batched = [
            (
                project_to_random_angle(
                    motion_batch.permute(0, 3, 1, 2),
                    self.args.dataset,
                    self.args.angle_mode,
                ).permute(0, 2, 3, 1),
                model_kwargs,
            )
            for motion_batch, model_kwargs in motions_batched
        ]
        motions_batched = [
            (self.encoder.transform(motion_batch.to(self.encoder.device)), model_kwargs)
            for motion_batch, model_kwargs in motions_batched
        ]
    ...
    def get_samples(self, subject, model_args=None):
        if "test_data" == subject:
            ...
            return self.get_3d_samples(
                MOTIONBERT_SAMPLES_PATH,
                scale=SCALE_FOR_SAVED_SAMPLES,
                flip=True,
                fps_ratio=2,
            )  # get motionBert samples

        elif "ElePose" == subject:
            return self.get_3d_samples(
                ELEPOSE_SAMPLES_PATH, scale=1, flip=False, fps_ratio=-2
            )  # get ElePose samples
    ...
    def visualize_samples(self, motions):
        if (
            self.args.vis_subjects is None
            or len(self.args.vis_subjects) == 0
            or self.args.vis_subjects[0] is None
            or self.args.vis_subjects[0] == ""
        ):
            return
        visualized = False
        for vis_subject in self.args.vis_subjects:
            if vis_subject in motions:
                for motion, model_kwargs in motions[vis_subject][
                    : ceil(self.args.num_visualize_samples / self.args.batch_size)
                ]:
                    visualized = True
                    date_time = datetime.now().strftime("%Y%m%d_%H%M%S")
                    name = f"{vis_subject}_sample_{date_time}"
                    print(f"visualizing to {name} a motion of length {motion.shape[1]}")
                    self.encoder.visualize(
                        motion.to(self.encoder.device),
                        name,
                        model_kwargs=model_kwargs,
                        normalized=True,
                        num_samples=self.args.num_visualize_samples,
                    )
        assert (
            visualized
        ), f"No samples were visualized, expected one of {self.args.vis_subjects} to be in the samples"
    ...
    def evaluate_samples(self, samples, test_samples):
        ...
        for motion_pair, test_motion_pair in zip(samples, test_samples):
            ...
            m_min_x = mx.min(axis=1).values
            m_max_x = mx.max(axis=1).values
            m_min_y = my.min(axis=1).values
            m_max_y = my.max(axis=1).values
            t_min_x = tx.min(axis=1).values
            t_max_x = tx.max(axis=1).values
            t_min_y = ty.min(axis=1).values
            t_max_y = ty.max(axis=1).values

            # Per-frame "diameter" (diagonal of bbox)
            m_diam = torch.sqrt(
                (m_max_x - m_min_x) ** 2 + (m_max_y - m_min_y) ** 2
            )  # (B, F,)
            t_diam = torch.sqrt(
                (t_max_x - t_min_x) ** 2 + (t_max_y - t_min_y) ** 2
            )  # (B, F,)
            # Accumulate ratios
        ...
        if "diversity" in self.args.metrics_names:
            all_metrics["diversity"] = calculate_diversity(
                encodings, diversity_times=min(len(test_encodings) - 1, DIVERSITY_TIMES)
            )

        if "precision" in self.args.metrics_names:
            all_metrics["precision"] = calculate_precision(
                encodings, test_encodings, k=PRECISION_AND_RECALL_K
            )

        if "recall" in self.args.metrics_names:
            all_metrics["recall"] = calculate_recall(
                encodings, test_encodings, k=PRECISION_AND_RECALL_K
            )
    ...
    def evaluate_multiple_times(self, model_args=None, print=True, save=True):
        all_metrics = {
            subject: {metric_name: [] for metric_name in self.args.metrics_names}
        ...
        if print:
            print_table(
                ...
                metrics_intervals,
                self.args.subjects,
                self.args.metrics_names,
            )

        if save:
            log_path = model_args.model_path.replace(".pt", ".pth").replace(
                ".pth", f"_eval_{self.args.angle_mode}.txt"
            )
            save_metrics(
                metrics_means,
                metrics_intervals,
...
def summarize_metrics(all_metrics, evaluator_args):
    total_metrics = {
        subject: {metric_name: [] for metric_name in evaluator_args.metrics_names}
        for subject in evaluator_args.subjects
    }
    ...
    total_means = {
        subject: {
            metric_name: np.mean(metrics)
            for metric_name, metrics in subject_all_metrics.items()
        }
        for subject, subject_all_metrics in total_metrics.items()
    }
    total_stds = {
        subject: {
            metric_name: np.std(metrics)
            for metric_name, metrics in subject_all_metrics.items()
        }
        for subject, subject_all_metrics in total_metrics.items()
    }
    conf_intervals = {
        subject: {
            metric_name: 1.96 * std / np.sqrt(evaluator_args.num_eval_iterations)
            for metric_name, std in subject_stds.items()
        }
        for subject, subject_stds in total_stds.items()
    }
...
def print_table(total_means, conf_intervals, subjects, metrics_names):
    print(
        " " * 24 + "".join([f"{metric_name: <20}" for metric_name in (metrics_names)])
    )
...
def save_metrics(
    metrics_means, metrics_intervals, log_file_path, subjects, metrics_names
):
