...
def parse_and_load_from_dataset():
    ...
    group.add_argument(
        "--dataset",
        default="nba",
        choices=["nba", "horse", "gymnastics"],
        type=str,
        help="Dataset name (choose from list).",
    )
    update_defaults(
        group,
        parse_and_load_from_path("model_path", "resume_checkpoint", "evaluator_path"),
    )
...
def add_base_options(parser: ArgumentParser):
    ...
    group.add_argument(
        "--seed", default=0, type=int, help="Seed for all random sampling."
    )
...
def add_diffusion_options(parser: ArgumentParser):
    ...
    group.add_argument(
        ...
        default="x_start",
        type=str,
        choices=["x_start", "epsilon"],
        help="Defines the model's prediction type. Can be 'x_start' or 'epsilon'.",
    )
    group.add_argument(
        "--noise_schedule",
        default="cosine_tau_2",
        type=str,
        help="Noise schedule type. Can be 'cosine', 'linear' or 'cosine_tau_{n}' (cosine(normalize(t))^n).",
    )
    group.add_argument(
        "--diffusion_steps",
        default=100,
    ...
    group.add_argument(
        ...
        default=True,
        type=bool,
        help="Controls the variance type of the backwards posterior.",
    )
...
def add_model_options(parser: ArgumentParser):
    ...
    group.add_argument(
        "--activation", default="gelu", type=str, help="Activation function."
    )
    ...
    group.add_argument(
        ...
        default=0.1,
        type=float,
        help="The probability of masking the condition when applying classifier-free guidance learning.",
    )
    update_defaults(group, parse_and_load_from_path("model_path", "resume_checkpoint"))
...
def add_evaluator_options(parser: ArgumentParser):
    ...
    group.add_argument(
        "--e_latent_dim", default=256, type=int, help="Transformer width."
    )
    group.add_argument(
        "--e_num_heads", default=4, type=int, help="Number of attention heads."
    )
    group.add_argument("--e_dropout", default=0.1, type=float, help="Dropout rate.")
    group.add_argument(
        "--e_activation", default="gelu", type=str, help="Activation function."
    )
...
def add_data_options(parser: ArgumentParser):
    ...
    group.add_argument(
        ...
        default="",
        type=str,
        help="The directory that holds the motions directory. If empty, will use defaults according to the specified dataset.",
    )
    group.add_argument(
        "--data_size",
        default=None,
        type=int,
        help="If specified, will use a subset of the dataset with the given size.",
    )
    group.add_argument(
        "--data_split",
        default="train",
    ...
    group.add_argument(
        ...
        nargs="+",
        type=str,
        choices=["shift_aug", "scale_aug", "length_aug", ""],
        help="Types of data augmentations to apply.",
    )
    group.add_argument(
        "--cond",
        default="no_cond",
        type=str,
        choices=["no_cond", "text", "action"],
...
def add_training_options(parser: ArgumentParser):
    ...
    group.add_argument(
        "--overwrite_model",
        action="store_true",
        help="If True, will enable to use an already existing output_dir, while deleting the original content.",
    )
    group.add_argument(
        "--train_platform_type",
        default="NoPlatform",
    ...
    group.add_argument(
        ...
        default="",
        type=str,
        help="If not empty, will start from the specified checkpoint (path to <model_name>.pth file).",
    )
    group.add_argument(
        "--train_batch_size", default=32, type=int, help="Batch size during training."
    )
...
def get_use_data():
    ...
    parser.add_argument(
        "--use_data",
        action="store_true",
        help="If specified, will use data / condition from dataset. You can override the conditions by specifying motion_length or text_prompt.",
    )
...
def add_sampling_options(parser: ArgumentParser):
    ...
    group.add_argument(
        ...
        default="",
        type=str,
        help="Path to <checkpoint_name>.pth file to be loaded.",
    )
    group.add_argument(
        "--output_dir",
        default="",
        type=str,
        help="Path to results dir (automatically created by the script). "
        "If empty, will create an output dir inside the model_path directory.",
    )
    group.add_argument(
    ...
    group.add_argument(
        ...
        default=2.5,
        type=float,
        help="Specifies the classifier-free guidance scale parameter.",
    )
    group.add_argument(
        "--overwrite",
        action="store_true",
    ...
    group.add_argument(
        "--show_input_motions",
        action="store_true",
        help="If True, will show the motions sampled from the data.",
    )
    group.add_argument(
        "--motion_length",
        default=6.0 if not get_use_data() else None,
        type=float,
    ...
    group.add_argument(
        ...
        default=None,
        type=str,
        help="A text prompt to use for conditioning.",
    )
...
def add_mas_options(parser: ArgumentParser):
    ...
    group.add_argument(
        "--num_views", default=7, type=int, help="Number of views optimize with."
    )
...
def add_ablation_options(parser: ArgumentParser):
    ...
    group.add_argument(
        ...
        choices=[
            "mas",
            "dreamfusion",
            "dreamfusion_annealed",
            "no_3d_noise",
            "prominent_angle",
            "prominent_angle_no_3d_noise",
            "random_angles",
        ],
...
def add_evaluation_options(parser: ArgumentParser):
    ...
    group.add_argument(
        "--subjects",
        default="motionBert ElePose train_data model mas dreamfusion no_3d_noise",
        type=str,
        help="Subjects to evaluate, separated by spaces.",
    )
    group.add_argument(
        "--vis_subjects",
        default="",
        type=str,
        help="Subjects to visualize, separated by spaces. For example: 'motionBert mas'.",
    )
    group.add_argument(
        "--num_visualize_samples",
        default=4,
        type=int,
        help="Number of samples to visualize for each subject.",
    )
    group.add_argument(
        "--metrics_names",
        default="fid diversity precision recall",
        type=str,
        help="Metrics to measure, separated by spaces.",
    )
    group.add_argument(
        "--evaluator_path",
        default="save/evaluator/nba_v2/no_aug/checkpoint_1000000.pth",
        type=str,
        help="Path to <model_name>.pth file to use for the evaluator.",
    )
    group.add_argument(
        "--num_eval_iterations",
        default=10,
        type=int,
        help="Number of iterations to run evaluation loop.",
    )
    group.add_argument(
        "--angle_mode",
        default="uniform",
        type=str,
        choices=["uniform", "side", "hybrid"],
        help="Horizontal angle sampling mode for projecting the 3D motions.",
    )
    group.add_argument(
        "--eval_num_samples",
        default=1024,
        type=int,
        help="Number of samples to load for each subject during evaluation loop. If you wish to compare to the results presented in the paper, do not change the default value.",
    )
    group.add_argument(
        "--saved_samples_path",
        default=None,
        type=str,
        help="Path to saved samples directory.",
    )
...
def add_evaluator_sampling_options(parser: ArgumentParser):
    ...
    group.add_argument(
        "--evaluator_sampling_mode",
        default="reconstruct",
        type=str,
        choices=["reconstruct", "generate"],
        help="Sampling mode for the evaluator.",
    )
...
def train_args():
    ...
    if get_eval_during_training():
        ...
        (
            evaluator_args.subjects,
            evaluator_args.vis_subjects,
            evaluator_args.metrics_names,
        ) = (
            evaluator_args.subjects.split(" "),
            evaluator_args.vis_subjects.split(" "),
            evaluator_args.metrics_names.split(" "),
        )
